...
const Bet = ({ index, add, setAdd }: BetProps) => {
	...
	const manualPlus = (amount: number, btnNum: BetOptType) => {
		...
		sounds.tick(); // click sound on quick amount select
		update(value);
	}
	...
	const onBetClick = (s: boolean) => {
		if (s) {
			...
			sounds.betPlaced(); // 🔊 bet placed sound
			toast.success(`Bet of ₹${betAmount.toFixed(2)} placed!`, { autoClose: 1500 });
		}
		...
		if (betted) {
			if (autoCashoutState) {
				if (cashOut < currentSecondNum) {
					...
					sounds.cashOut(); // 🔊 auto cashout sound
				}
			}
		}
	...
	const handleCashOut = () => {
		const winAmount = Number(betAmount * currentTarget).toFixed(2);
		...
		if (currentTarget >= 10) {
			sounds.bigWin();
			toast.success(`🚀 BIG WIN! ₹${winAmount} at ${currentTarget.toFixed(2)}x!`, { autoClose: 4000 });
		} else {
			toast.success(`✅ Cashed out ₹${winAmount} at ${currentTarget.toFixed(2)}x`, { autoClose: 2000 });
		}
	}
				...
				<div className="first-row">
					<div className="bet-block">
						<div className="bet-spinner">
							<div className={`spinner ${betState || betted ? "disabled" : ""}`}>
								...
								<div className="input">
									{betState || betted ?
										<input type="number" value={Number(myBetAmount)} readOnly /> :
										<input type="number" value={Number(myBetAmount)}
											onChange={e => {
												Number(e.target.value) > maxBet
													? update({ ...state, userInfo: { ...state.userInfo, [`${index}`]: { betAmount: maxBet } } })
													: Number(e.target.value) < 0
														? update({ ...state, userInfo: { ...state.userInfo, [`${index}`]: { betAmount: 0 } } })
						...
						{betState || betted ?
							<div className="bet-opt-list">
								{['20','50','100','1000'].map(v => (
									<button key={v} className="bet-opt disabled"><span>{v}</span></button>
								))}
							</div> :
							<div className="bet-opt-list">
								<button onClick={() => manualPlus(20, "20")} className="bet-opt"><span>20</span></button>
								<button onClick={() => manualPlus(50, "50")} className="bet-opt"><span>50</span></button>
								<button onClick={() => manualPlus(100, "100")} className="bet-opt"><span>100</span></button>
								<button onClick={() => manualPlus(1000, "1000")} className="bet-opt"><span>1000</span></button>
							</div>
						}
					</div>
					...
					<div className="buttons-block">
						{betted ? GameState === "PLAYING" ?
							<button className="btn-waiting" onClick={handleCashOut}>
								<span>
									<label>CASHOUT</label>
							...
							<div className="modal-body">
								<div className="content-part content-part-1">
									...
									<div className="rounds-wrap">
										{[10, 20, 50, 100].map(n => (
											<button key={n} className={`btn-secondary ${autoCound === n ? "onClick" : ""}`}
								...
								<div className="content-part">
									<div className={`input-switch ${deState ? "" : "off"}`}
										onClick={() => update({ [`${index}deState`]: !deState, [`${index}decrease`]: 0 })}>
										<span className="oval"></span>
									</div>
									<span className="title">Stop if cash decreases by</span>
									<div className="spinner">
										<div className={`m-spinner ${deState ? "" : "disabled"}`}>
											<div className="buttons"><button onClick={() => deState && minus("decrease")} className="minus"></button></div>
											<div className="input">
												<input type="number" onChange={(e) => deState && update({ [`${index}decrease`]: Number(e.target.value) })} value={decrease} readOnly={!deState}
													onBlur={(e) => onChangeBlur(Number(e.target.value) || 0, "decrease")} />
											</div>
											<div className="buttons"><button onClick={() => deState && plus("decrease")} className="plus"></button></div>
										</div>
									</div>
									<span>INR</span>
								</div>
								<div className="content-part">
									<div className={`input-switch ${inState ? "" : "off"}`}
										onClick={() => update({ [`${index}inState`]: !inState, [`${index}increase`]: 0 })}>
										<span className="oval"></span>
									</div>
									<span className="title">Stop if cash increases by</span>
									<div className="spinner">
										<div className={`m-spinner ${inState ? "" : "disabled"}`}>
											<div className="buttons"><button onClick={() => inState && minus("increase")} className="minus"></button></div>
											<div className="input">
												<input type="number" onChange={(e) => inState && update({ [`${index}increase`]: Number(e.target.value) })} value={increase} readOnly={!inState}
													onBlur={(e) => onChangeBlur(Number(e.target.value), "increase")} />
											</div>
											<div className="buttons"><button onClick={() => inState && plus("increase")} className="plus"></button></div>
										</div>
									</div>
									<span>INR</span>
								</div>
								<div className="content-part">
									<div className={`input-switch ${single ? "" : "off"}`}
										onClick={() => update({ [`${index}single`]: !single, [`${index}singleAmount`]: 0 })}>
										<span className="oval"></span>
									</div>
									<span className="title">Stop if single win exceeds</span>
									<div className="spinner">
										<div className={`m-spinner ${single ? "" : "disabled"}`}>
											<div className="buttons"><button onClick={() => single && minus("singleAmount")} className="minus"></button></div>
											<div className="input">
												<input type="number" onChange={(e) => single && update({ [`${index}singleAmount`]: Number(e.target.value) })} value={singleAmount} readOnly={!single}
													onBlur={(e) => onChangeBlur(Number(e.target.value), "singleAmount")} />
											</div>
											<div className="buttons"><button onClick={() => single && plus("singleAmount")} className="plus"></button></div>
										</div>
									</div>
									<span>INR</span>
								</div>
